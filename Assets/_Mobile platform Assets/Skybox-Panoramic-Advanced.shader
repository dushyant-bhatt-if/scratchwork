...
// Extended by 3 axis rotation

Shader "Skybox/Panoramic-Advanced" {
		Properties{
			_Tint("Tint Color", Color) = (.5, .5, .5, .5)
			_BackgroundColor("Background Color", Color) = (.5, .5, .5, .5)
...
#include "UnityCG.cginc"
		...
		fixed4 frag(v2f i) : SV_Target
		{
			...
			c *= _Exposure;
			//return half4(c, 1);
			return half4(lerp(_BackgroundColor.rgb, c.rgb, tex.a), 1);
			...
			CustomEditor "SkyboxPanoramicAdvancedShaderGUI"
			Fallback Off
